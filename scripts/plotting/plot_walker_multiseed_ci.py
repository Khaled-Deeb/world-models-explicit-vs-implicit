...
FIG_DIR = BASE / "figures" / "walker_ci"
FIG_DIR.mkdir(parents=True, exist_ok=True)

DREAMER_RUNS = {
    "seed1": BASE / "runs" / "dreamer_walker_gpu_seed1_100k",
    "seed2": BASE / "runs" / "dreamer_walker_gpu_seed2_100k",
    "seed3": BASE / "runs" / "dreamer_walker_gpu_seed3_100k",
}

TD_RUNS = {
    "seed1": BASE / "tdmpc2" / "tdmpc2" / "logs" / "walker-walk" / "1" / "walker_200k_seed1",
    "seed2": BASE / "tdmpc2" / "tdmpc2" / "logs" / "walker-walk" / "2" / "walker_100k_seed2",
    "seed3": BASE / "tdmpc2" / "tdmpc2" / "logs" / "walker-walk" / "3" / "walker_100k_seed3",
}

GRID = np.arange(5000, 95000 + 1, 5000, dtype=float)
...
def load_td_eval(path: Path) -> pd.DataFrame:
    ...
    df = df.rename(columns={reward_col: "episode_reward"})
    df = df.drop_duplicates(subset="step", keep="last")
    return df
...
plt.title("DreamerV3 walker-walk: mean and 95% CI (3 seeds)")
plt.legend(loc="upper left")
plt.tight_layout()
plt.savefig(FIG_DIR / "dreamer_walker_mean_ci.png", dpi=160)
plt.close()
...
plt.title("TD-MPC2 walker-walk: mean and 95% CI (3 seeds, up to 95k)")
plt.legend(loc="upper left")
plt.tight_layout()
plt.savefig(FIG_DIR / "tdmpc2_walker_mean_ci.png", dpi=160)
plt.close()
...
plt.title("Walker-walk: Dreamer vs TD-MPC2 (mean ± 95% CI, 3 seeds)")
plt.legend(loc="upper left")
plt.tight_layout()
plt.savefig(FIG_DIR / "dreamer_vs_tdmpc2_walker_mean_ci.png", dpi=160)
plt.close()

summary = [
    f"Grid max step: {int(GRID.max())}",
    f"Dreamer final mean @95k: {dreamer_mean[-1]:.4f}",
    f"Dreamer final 95% CI half-width @95k: {dreamer_ci[-1]:.4f}",
    f"TD-MPC2 final mean @95k: {td_mean[-1]:.4f}",
    f"TD-MPC2 final 95% CI half-width @95k: {td_ci[-1]:.4f}",
